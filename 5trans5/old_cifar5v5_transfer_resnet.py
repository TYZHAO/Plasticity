...
def fetch_data(fetch, data, label):
...
def train_data_loader():
    ...
    return fetch_data(use_train, input_data, input_label)
...
def resnet_model(features, labels, mode):
    ...
    pretrain_root = '/scratch/tz1303/ckpts_5v5_train_on_1/'
    pretrained_model=os.path.join(pretrain_root, FLAGS.load_dir)
    tf.train.init_from_checkpoint(pretrained_model, {'feature_extractor/':'transfer/'})
    ...
    if mode == tf.estimator.ModeKeys.TRAIN:
        epoch = tf.ceil(global_step*BATCH_SIZE/data_amount)#+PRETRAIN_EPOCH
...
def inference():
    ...
    model_root = '/scratch/tz1303/ckpts_5v5_transfer_to_2'
