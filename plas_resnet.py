...
def resnet_layer(inputs, scope_name, update_ops, filters=16, kernel_size=3, strides=1, 
    ...
    with tf.variable_scope(scope_name):
        ...
        hebb_new = hebb_new/(shapey[0] * shapey[1] * shapey[2]) - tf.reduce_mean(tf.square(tf.tanh(x)), 
            axis=[0,1,2,3]) *hebb
...
def resnet_model(features, scope, num_res_blocks, classcount):
    ...
    with tf.variable_scope('classifier'):
        ...
        hebb_new = tf.reduce_mean(tf.matmul(tf.expand_dims(tf.tanh(x), axis=-1),tf.expand_dims(tf.tanh(y), axis=1)), axis=0)
        hebb_new = hebb_new - hebb*tf.reduce_mean(tf.square(tf.tanh(y)), axis=[0,1])
        hebb_ = tf.assign(hebb_update, hebb_new)
